...
public class Sekibune : Ship
{
    ...
    {
        foreach (var i in GameManager.Instance.allyList)
            if (Vector3.Distance(transform.position, i.transform.position) <= 20)
                i.GetComponent<Ship>().shipData.shipHpData.hp -= i.GetComponent<Ship>().shipData.shipHpData.maxHp / 8 / 3 * Time.deltaTime;
    }
